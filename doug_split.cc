...
void dump_memory(std::ostream &f, std::string indent, Mem &mem)
{
  ...
  for (int ridx = 0; ridx < GetSize(mem.wr_ports); ridx++)
  {
    ...
    if (root.clk_enable) {
      f << stringf("%s" "always%s @(%sedge ", indent.c_str(), false ? "_ff" : "", root.clk_polarity ? "pos" : "neg");
      dump_sigspec(f, root.clk);
      f << ") begin\n";
    } else {
      f << stringf("%s" "always%s begin\n", indent.c_str(), false ? "_latch" : " @*");
    }
  ...
  for(auto &pair : clk_to_lof_body)
  {
    ...
    if( clk_domain != "")
    {
      f << stringf("%s" "always%s @(%s) begin\n", indent.c_str(), false ? "_ff" : "", clk_domain.c_str());
...
#undef HANDLE_BINOP
  ...
  {
    f << stringf("%s" "always%s if (", indent.c_str(), false ? "_comb" : " @*");
    dump_sigspec(f, cell->getPort(ID::EN));
...
void dump_cell(std::ostream &f, std::string indent, RTLIL::Cell *cell)
{
  ...
  if (!false && cell->parameters.size() > 0) {
    f << stringf(" #(");
  ...
  if (false && RTLIL::builtin_ff_cell_types().count(cell->type) && cell->hasPort(ID::Q) && !cell->type.in(ID($ff), ID($_FF_))) {
    std::stringstream ss;
...
bool split_ff(std::ostream &f, std::string indent, RTLIL::Cell *cell)
{
...
void dump_process(std::ostream &f, std::string indent, RTLIL::Process *proc, bool find_regs = false)
{
  ...
  for (size_t i = 0; i < proc->syncs.size(); i++)
  {
    ...
    if (sync->type == RTLIL::STa) {
      ...
      f << stringf("%s" "always%s @(", indent.c_str(), false ? "_ff" : "");
...
void dump_module(std::ostream &f, std::string indent, RTLIL::Module *module)
{
  ...
  f << stringf(");\n");
  if (!false && !module->processes.empty()) {
    initial_id = NEW_ID;
...
struct DougCmd : public Pass {

  DougCmd() : Pass("doug", "do whatever Doug wants") { }

  void help() override
  {
    //   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
    log("\n");
    log("    doug <destmod> <srcmod> <cycle number> [options]\n");
    log("\n");
  ...
  void execute(std::vector<std::string> args, RTLIL::Design *design) override
  {
    ...
    for (auto cell : destmod->cells()) {
      split_ff(f, indent + "  ", cell);
